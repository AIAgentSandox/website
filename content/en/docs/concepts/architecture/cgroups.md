...
* If you deploy [Node.js](https://nodejs.org/) applications, prefer to use versions that detect cgroup v2
  resource limits. Node.js reads cgroup v2 CPU and memory limits (through [libuv](https://libuv.org/))
  starting with Node.js v20. Earlier versions may read the host's CPU count and memory instead of the
  limits applied to the pod, which can lead to an incorrectly sized heap and out-of-memory (OOM)
  terminations. As a workaround on older versions, set the heap size explicitly, for example with the
